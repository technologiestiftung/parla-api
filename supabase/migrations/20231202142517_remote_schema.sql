...
BEGIN
	RETURN query WITH chunk_winners AS(
		SELECT
			...
			cw.similarity
		FROM
			match_document_chunks(embedding, match_threshold, chunk_limit, num_probes_chunks) AS cw
),
summary_winners AS(
	SELECT
		NULL AS chunk_id,
		...
		sw.similarity
	FROM
		match_summaries(embedding, match_threshold, summary_limit, num_probes_summaries) AS sw
),
all_winners AS(
	SELECT

...
BEGIN
	...
	WITH chunk_winners AS (
	    SELECT
	        ...
	        cw.similarity
	    FROM match_document_chunks(embedding, match_threshold, chunk_limit, 0, num_probes) as cw
	), summary_winners AS (
	    SELECT
	        NULL AS chunk_id,
	        sw.id AS summary_id,
	        ...
	        sw.similarity
	    FROM match_summaries(embedding, match_threshold, summary_limit, 0, num_probes) as sw
	), all_winners AS (

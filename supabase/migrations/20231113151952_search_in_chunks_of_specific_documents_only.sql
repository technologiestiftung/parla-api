...
CREATE OR REPLACE FUNCTION public.match_document_chunks_for_specific_documents(processed_document_ids integer[], embedding vector, match_threshold double precision, match_count integer, min_content_length integer, num_probes integer)
...
BEGIN
	...
	SELECT
		...
		WHERE
			processed_document_chunks.processed_document_id = ANY(processed_document_ids) and

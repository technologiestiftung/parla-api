...
export interface ParlaConfig extends FastifyPluginOptions {
	...
	CHAT_COMPLETION_GENERATED_TOKEN_LIMIT: number;
...
async function main() {
	try {
		...
		const CHAT_COMPLETION_GENERATED_TOKEN_LIMIT =
			process.env.CHAT_COMPLETION_GENERATED_TOKEN_LIMIT;
		...
		if (!CHAT_COMPLETION_GENERATED_TOKEN_LIMIT) {
			throw new EnvError("OPENAI_EMBEDDING_MODEL");
		}
		if (
		...
		if (
			!CHAT_COMPLETION_GENERATED_TOKEN_LIMIT ||
			isNaN(parseInt(CHAT_COMPLETION_GENERATED_TOKEN_LIMIT))
		) {
			throw new EnvError("CHAT_COMPLETION_GENERATED_TOKEN_LIMIT");
		}
		if (
		...
		const parlaConfig = {
			...
			CHAT_COMPLETION_GENERATED_TOKEN_LIMIT: parseInt(
				CHAT_COMPLETION_GENERATED_TOKEN_LIMIT,
			),
